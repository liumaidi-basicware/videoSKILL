...
**当前素材阶段依赖规则**：禁止一次性生成全部素材。统一使用 `storyboard.py --stage next --run-id <稳定版本ID>` 逐阶段推进：已确认用户产品图 → 产品九宫格板 → 客户确认产品板 → 人物六视图板 → 客户确认人物板 →（数字人与产品同时存在时）人物产品使用细节图 → 客户确认产品使用图 → 分段故事板 → 客户确认 → 视频。产品使用图必须同时引用已确认人物板与产品板，清楚展示人物真实使用产品的动作、手部接触点、操作关系和产品关键细节，并作为后续故事板的高优先级参考素材。产品板必须通过 BasicRouter 异步 `/v1/image-generations` 的 `imageUrls` 做 img2img，并绑定源产品图指纹；源图变化后旧 `product_board_pending.jpg` 与旧确认必须失效。任何历史段落中“一次命令连续生成产品板/人物板/故事板”的说明均视为旧流程。

**产品使用图逐格生成规则**：对于存在磁吸、卡扣、佩戴、插接、承重或其他高风险接触关系的使用图，禁止让模型一次性绘制 3×3 九宫格。系统必须为每个 panel 独立调用异步图像生成，复用同一产品身份锚和物理关系合同，再由本地无模型工具拼版。拼版图仅用于客户确认；结果 JSON 必须保留每格的 `panel_index`、本地路径和 BasicRouter retrieve URL，确认前必须检查 9 个 panel 和 9 个远程 URL 完整。第 3、4、7 等关键接触格必须逐格证明目标接触面的宽平面、贴合接触线和产品外露面，不能用目标物边缘、屏幕或上沿替代接触面。
...
5a. **故事板转视频铁律（2026-08-06 修订：Seedance 原生故事板优先，Kling 单格展开兜底）**：确认后的 `shot_*.jpg` 故事板整图（格数 N 由剧本决定，不再固定12格）是客户确认过的分镜合同。生成视频时必须**逐 shot 单独处理**，但提交素材要按目标模型能力分流：① 首选 Seedance 时，使用 `storyboard_ref_mode:"native_storyboard"`，把确认故事板/contact sheet 与该 shot 的 `ref_tags[]` 对应素材一起提交；提示词必须写明当前 `segment/panel_index`，只执行当前镜头，同时允许 Seedance 读取整张故事板的前后镜头关系、构图递进、动作节奏、光线和情绪弧线来保证多片段连贯。② 只有 Seedance 不可用、隐私策略拒绝或能力不足而回落 Kling 时，才为该 shot 生成聚焦分镜的高分辨率「单格展开图」（基于故事板整图 + `panel_index` + `ref_tags[]`，用 `gpt-image-2` img2img 重新生成，而非直接像素裁剪）；Kling 提交时使用 `storyboard_ref_mode:"expanded_panel"`，并把单格展开图连同该 shot 的确认素材作为 `imageUrls`。③ 视频提交文本必须采用逐镜头 `@tag` 内联句式（"镜头N，X秒，@host 拿起 @product……"），明确该分镜具体演什么、用哪些素材图；Kling 路径不得依赖"按1→N顺序演"的整图猜测规则。④ 每个 shot 独立发起一次视频生成调用（`video_type` 与并行提交策略同铁律#7，多段仍先一次性提交再统一轮询；需要强连续性时用 `--chain` 尾帧串联）。⑤ 严格保持角色/产品/场景/光线、普通话声音人设、BGM 氛围和 SFX 强弱在各分镜间一致；不要添加额外角色；不要改变剧情、服装、道具、产品外观和场景关系；相邻分镜仍遵守铁律#5 的机位/景别跨度要求。`--storyboard-ref` 参数标识"该 shot 使用已确认故事板作为分镜来源"，具体走 Seedance native 还是 Kling expanded panel 由 `video_engine.py` 按实时模型能力决定。
